...
import { Users, Plus, Trash2, ShieldCheck, Mail, Sparkles, X } from 'lucide-react';
              ...
              <tr key={usr.id} className="hover:bg-slate-800/40 transition-colors">
                <td className="py-3 px-3">
                  <div className="flex items-center gap-3">
                    <img src={usr.avatar} alt={usr.name} className="w-9 h-9 rounded-full object-cover border border-amber-500/40" />
                ...
                <td className="py-3 px-3 text-slate-400 font-mono">{usr.lastLogin || 'Never'}</td>
          ...
          <div className="w-full max-w-md bg-[#0B1F2A] border border-[#D4AF37]/40 rounded-3xl p-6 relative shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-amber-400" /> Add Admin User
              </h3>
